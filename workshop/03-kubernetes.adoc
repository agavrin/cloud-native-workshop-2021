...
Ultimately, we define our environment declaratively, using Infrastructure-as-Code, similar to what we did using the `podmanfile` files, but this time for the whole environment.
...
That is, that the podman containers we had in the last part will now run inside Kubernetes pods, one pod per container instance.
...
The snippet defines the specification of one pod that should contain a single container which is created from our coffee-shop podman image.
...
If we want to scale our application, change the deployment definition in another way, or deploy a different version (that is podman image), we simply change the YAML definition and update it against the cluster.
...
With MicroProfile and its programming model, combined with containers and Kubernetes, we can easily build 12-factor microservices.
